...
1. 理解 ZMQ 三种通信模式（REQ/REP、DEALER/ROUTER、PULL/PUSH）在 vLLM 中各自适合的场景
2. 掌握 `EngineCoreProc` 输入/输出 socket 线程如何把 ZMQ I/O 与引擎计算解耦
3. 理解 `AsyncLLM` 如何通过 `AsyncMPClient` 把请求发送到后台 `EngineCore`
4. 掌握 `output_handler`、`OutputProcessor`、`RequestOutputCollector` 三者的协作方式
5. 理解 vLLM 在通信层里使用的零拷贝、buffer 复用、事件循环让渡等性能优化技巧
